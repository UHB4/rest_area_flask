...
@app.route('/api/gas-stations-and-details', methods=['GET'])
def get_gas_stations_and_details():
    code = request.args.get('code')
    out = request.args.get('out')
    osnm = request.args.get('osnm')
    area = request.args.get('area')
    ...
    try:
        gas_stations_response = requests.get('http://localhost:5000/api/gas-stations', params={
            'code': code,
            'out': out,
            'osnm': osnm,
            'area': area
        })
        gas_stations_response.raise_for_status()
        gas_stations = gas_stations_response.json()
        uni_ids = [station['uni_id'] for station in gas_stations]

        details = []
        for uni_id in uni_ids:
            detail_response = requests.get('http://localhost:5000/api/gas-station-detail', params={
                'uni_id': uni_id
            })
            detail_response.raise_for_status()
            details.append(detail_response.json())

        return jsonify({'gas_stations': gas_stations, 'details': details})
    except requests.exceptions.RequestException as e:
        return jsonify({'error': 'Failed to retrieve gas stations and details'}), 500
